...
#define nvc_stats_args_start_time ((uint64_t)1 << 1)
#define nvc_stats_args_end_time ((uint64_t)1 << 2)
...
typedef void (*nvOS_msg_name_func_t)(void *arg, const char *str);
typedef void (*nvOS_cancel_func_t)(void *arg);
...
typedef struct nvOS_io_s {
	...
	void * status_arg;
	nvOS_msg_name_func_t msg_name_func;
	void *msg_name_arg;
	/* for list input, count num marshalled so far */
	int  marshalled_count;
	/* for list input, mark when done */
	boolean_t unmarshal_done;
	/* cancel func is used to cancel list input sends */
	nvOS_cancel_func_t cancel_func;
	void *cancel_arg;
} nvOS_io_t;
...
typedef struct nvc_conn_s {
	...
	nvc_name_string_t       conn_egress_vnet;
	nvc_pcl_vlan_id_t       conn_egress_vlan;
	...
	uint64_t                conn_syn_resends;
	uint64_t                conn_syn_ack_resends;
	hrtime_t                conn_dur;
	hrtime_t                conn_avg_latency;
	uint64_t                conn_bytes_sent;
	uint64_t                conn_bytes_recv;
	uint64_t                conn_bytes_total;
